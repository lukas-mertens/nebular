...
export abstract class NbBasePicker<D, T, P> extends NbDatepicker<T, D> {
  ...
  protected checkFormat() {
    if (this.dateService.getId() === 'native' && this.format) {
      throw new Error(
        "Can't format native date. To use custom formatting you have to install @devsaur-nebular/moment or " +
          '@devsaur-nebular/date-fns package and import NbMomentDateModule or NbDateFnsDateModule accordingly.' +
